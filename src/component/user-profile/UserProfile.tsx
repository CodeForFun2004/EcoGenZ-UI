...
const UserProfile: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAchievements = async () => {
      if (activeTab === "achievements" && user?.userId) {
        try {
          const res = await fetch(
            `${
              import.meta.env.VITE_API_BASE_URL
            }/api/achievements/get-by-user/${user.userId}`
          );
          if (!res.ok) throw new Error("Failed to fetch achievements");
          const data = await res.json();
          console.log("Achievements:", data);
          // Set to state to render dynamically if needed
        } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Only send if no new image is uploaded
    if (!selectedFile && formData.profilePhotoUrl) {
      formDataToSend.append("ProfilePhotoUrl", formData.profilePhotoUrl);
    }

    if (selectedFile) {
      formDataToSend.append("imageFile", selectedFile); // File is prioritized
    }

    try {
      const response = await fetch(
        `${import.meta.env.VITE_AUTH_URL}/update-user`,
        {
          method: "PUT",
          body: formDataToSend,
        }
      ...
      // Optionally dispatch updated user to Redux
    } catch (error) {
